...
class WcDonationOrder {
	...
	public function wc_donation_create_report ( $order_id ) {
		...
		if ( ! get_post_meta( $order_id, 'wc_donation_report_generated', true ) ) { 
			...
			update_post_meta( $order_id, 'wc_donation_report_generated', true );
		}
	}
	...
	public function addCartItemMetaToOrderItemMeta( $item_id, $item_values, $item_key ) {
		...
		wc_update_order_item_meta( $item_id, 'gift_aid', sanitize_text_field( $item_values['gift_aid'] ) );
		wc_update_order_item_meta( $item_id, 'tribute', sanitize_text_field( $item_values['tribute'] ) );
	...
	public function displayCartItemCompaignMeta( $item_data, $cart_item ) {
		...
		if ( !empty( $cart_item['gift_aid'] ) && '' !== $cart_item['gift_aid']) {
		...
		if ( !empty( $cart_item['tribute'] ) && '' !== $cart_item['tribute']) {
